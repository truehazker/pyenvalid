...
class TestConfigurationErrorMessageFormatting:
    ...
    def test_unicode_in_custom_title(self) -> None:
        ...
        error = ConfigurationError(
            [("api_key", "missing")], title="⚠️ CONFIGURATION ERROR ⚠️"
        )
        message = str(error)
    ...
    def test_unicode_in_custom_hint(self) -> None:
        ...
        error = ConfigurationError(
            [("api_key", "missing")], hint="💡 Check your .env file"
        )
        message = str(error)
    ...
    def test_multiple_errors_same_field(self) -> None:
        ...
        error = ConfigurationError([("api_key", "missing"), ("api_key", "string_type")])
...
class TestTerminalWidthHandling:
    ...
    def test_get_terminal_width_fallback(self) -> None:
        ...
        # Test the fallback logic by calling the static method directly
        # The implementation catches exceptions and returns 80
        width = ConfigurationError._get_terminal_width()

        # Should return a positive integer (either actual terminal width or fallback)
        assert isinstance(width, int)
        assert width > 0

    def test_message_formatting_narrow_terminal(self) -> None:
        """Should format message correctly in narrow terminal."""
        # Test with narrow terminal by passing small width to _BoxFormatter
        formatter = _BoxFormatter(40)

        # Should use minimum width
        assert formatter.box_width >= 30  # min_width default
    ...
    def test_message_formatting_wide_terminal(self) -> None:
        """Should format message correctly in wide terminal."""
        # Test with wide terminal by passing large width to _BoxFormatter
        formatter = _BoxFormatter(200)

        # Should clamp to max width
        assert formatter.box_width <= 80  # max_width default
